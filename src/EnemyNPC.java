...
public class EnemyNPC {
    ...
    public EnemyNPC randomNPC(Character character){
        ...
        npc.equipped.setBoot(RandomItems.randomWeapon(0));
        ...
        if (npc.getEquipped().getMain().getRange()>5){
            ...
            npc.baseDamageShort = 1+(character.getAttributes().getDifficulty()+1);
        }
        else {
            npc.baseDamageLong = 1+(character.getAttributes().getDifficulty()+1);
            npc.baseDamageShort = 2+(character.getAttributes().getDifficulty()+1);
        }
        npc.getInventory().addMoney(random.nextInt(npc.getMaxHealth()*(character.getAttributes().getLuck()+1))+1);
        return npc;
    }
